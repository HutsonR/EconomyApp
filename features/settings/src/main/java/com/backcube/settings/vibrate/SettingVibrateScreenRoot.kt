...
internal fun SettingVibrateScreen(
    ...
    Column(
        ...
        CustomListItem(
            ...
            trailingContent = {
                Switch(
                    ...
                    onCheckedChange = {
                        onIntent(SettingVibrateIntent.ToggleHaptics(it))
                    }
                )
            }
        )
        ...
        if (state.isHapticsEnabled) {
            ...
            state.hapticAvailableEffects.forEach {
                CustomOption (
                    text = context.getString(it.labelRes),
                    selected = state.currentHapticEffect == it.hapticEffect,
                    onSelect = { onIntent(SettingVibrateIntent.ChangeHapticsLevel(it.hapticEffect)) }
                )
                Spacer(modifier = Modifier.height(16.dp))
            }
        }
