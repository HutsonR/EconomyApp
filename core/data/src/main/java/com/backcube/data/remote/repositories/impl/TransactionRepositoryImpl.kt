...
import com.backcube.data.remote.models.request.transactions.toApiModel
import com.backcube.data.remote.models.response.transactions.toDomain
import com.backcube.data.remote.repositories.mappers.toApiDate
...
class TransactionRepositoryImpl @Inject constructor(
    ...
    override suspend fun createTransaction(request: TransactionRequestModel): TransactionResponseModel = retryHandler.executeWithRetry {
        transactionsApi.createTransaction(request.toApiModel()).getOrThrow().toDomain()
    }

    override suspend fun getTransactionById(id: Int): TransactionResponseModel = retryHandler.executeWithRetry {
    ...
    ): TransactionResponseModel = retryHandler.executeWithRetry {
        transactionsApi.updateTransaction(id, request.toApiModel()).getOrThrow().toDomain()
    }

    override suspend fun deleteTransaction(id: Int): Boolean = retryHandler.executeWithRetry {
    ...
    override suspend fun getAccountTransactions(
        ...
        startDate: Instant?,
        endDate: Instant?
    ): List<TransactionResponseModel> = retryHandler.executeWithRetry {
