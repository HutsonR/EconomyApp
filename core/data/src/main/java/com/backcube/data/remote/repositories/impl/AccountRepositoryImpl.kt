...
class AccountRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAccounts(): List<AccountModel> = retryHandler.executeWithRetry {
        accountApi.getAccounts().getOrThrow().map { it.toDomain() }
    }

    override suspend fun createAccount(request: AccountCreateRequestModel): AccountModel = retryHandler.executeWithRetry {
        accountApi.createAccount(request.toApi()).getOrThrow().toDomain()
    }

    override suspend fun getAccountById(id: Int): AccountResponseModel? = retryHandler.executeWithRetry {
    ...
    ): AccountModel = retryHandler.executeWithRetry {
        accountApi.updateAccount(id, request.toApi()).getOrThrow().toDomain()
    }

    override suspend fun getAccountHistory(id: Int): AccountHistoryResponseModel = retryHandler.executeWithRetry {
