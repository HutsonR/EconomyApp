...
class CategoryRepositoryImpl @Inject constructor(
    ...
    override suspend fun getCategories(): List<CategoryModel> = retryHandler.executeWithRetry {
        categoriesApi.getCategories().getOrThrow().map { it.toDomain() }
    }

    override suspend fun getCategoriesByType(isIncome: Boolean): List<CategoryModel> = retryHandler.executeWithRetry {
